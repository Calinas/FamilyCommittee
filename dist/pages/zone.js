...
var Zone = /** @class */ (function (_super) {
    ...
    function Zone() {
        ...
        _this_1.$events = { "CurrentModal": { "v-on:cancel": "commentCancel", "v-on:sure": "commentSure", "v-on:input": "bindCommentInput" }, "SelectModal": { "v-on:cancel": "toggleFlag", "v-on:sure": "selectSure" }, "shareModal": { "v-on:cancel": "toggleFlag", "v-on:sure": "toggleFlag" }, "SureModal": { "v-on:cancel": "toggleFlag", "v-on:sure": "toggleFlag" }, "deleteRecordModal": { "v-on:cancel": "deleteCancel", "v-on:sure": "deleteSure", "v-on:input": "bindDeleteInput" } };
        _this_1.components = {
            ...
            deleteRecordModal: commentModal_1.default
        };
        _this_1.data = {
            menuList: [
                {
        ...
        _this_1.methods = {
            ...
            bindDeleteInput: function (value) {
    ...
    Zone.prototype.onLoad = function () {
        if (!this.checkDataExist('memberInfo')) {
            ...
            this.memberInfo = wx.getStorageSync('memberInfo');
            if (this.memberInfo && this.memberInfo.member_id === -1)
                return; //微信测试用户
            this.$parent.globalData.userData = this.memberInfo;
            this.$apply();
            this.getZoneList();
    ...
    Zone.prototype.removeRecordFn = function (id, idx) {
        var _this_1 = this;
        zone_1.deleteRecord({
            moment_id: id,
            class_id: this.classInfo.id,
            ...
            if (res.data.success) {
                common_1.showMsg('撤销成功');
                _this_1.list[idx].deleted_at = '刚刚';
                _this_1.list[idx].info = __assign({}, _this_1.list[idx].info, { undo_msg: _this_1.deleteRecordInput });
                _this_1.deleteRecordInput = '';
                _this_1.$apply();
            }
        });
    };
